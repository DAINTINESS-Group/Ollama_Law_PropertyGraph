...
def graphPrompt(input_json: str, metadata={}, model="mistral-openorca:latest"):
    ...
    SYS_PROMPT = (
        ...
        "### STEP 1: **Process Entity List**\n"
        "You have been provided with a list of valid entities:\n"
        f"{entities_text}\n\n"

        "THINK: Before extracting any terms:\n"
        "   - Are BOTH terms present in the predefined list? \n"
        "   - If NOT, skip them entirely \n"
        "   - Nodes must exactly match the predefined list — no generalization, no inference.\n\n"

        "### STEP 2: **Extract Terms and Relationships**\n"
        "1. Nodes (terms):\n"
        "   - Node labels **MUST** be short (2-5 words) — CASE SENSITIVE.\n"
        "   - Nodes **MUST** match the predefined entity list — CASE SENSITIVE.\n"
        ...
        "3. STRICT RULES:\n"
        "   - If BOTH nodes are not from the predefined entity list → SKIP\n"
        "   - If no valid relationship exists → Return an **empty array** `[]`\n\n"
        " **Normalize Terms**\n"
        "- Remove pluralization where possible.\n"
        ...
        "- Treat nodes that differ only by qualifiers, pluralization, or minor modifiers as the same node (e.g., general-purpose AI model vs. general-purpose AI models).\n"
        "- Ensure consistent naming for entities across the entire document.\n"
        ...
        "- Use the paragraph_id from the input metadata.\n"
        "- The paragraph_id should appear alongside the extracted relationship terms.\n"

        "### STEP 4: **Encourage Cross-Connections**\n"
        ...
        "- Do not limit connections to entities within the same paragraph — connect across the document.\n"


        "Return the extracted relationships in this JSON format:\n"
        "[\n"
        "   {\n"
        '       "node_1": "Entity from the predefined list",\n'
        '       "node_2": "Another entity from the predefined list",\n'
        ...
        " **WARNING:**\n"
        "- If terms are not in the predefined list → EXCLUDE THEM\n"
        "- If no valid relationship → RETURN AN EMPTY ARRAY `[]`\n"
        "- If node labels and edge labels exceed 5 words → TRIM THEM TO 2-5 words\n"
